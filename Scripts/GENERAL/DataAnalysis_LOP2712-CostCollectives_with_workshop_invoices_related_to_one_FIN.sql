-- DataAnalysis_LOP2712-CostCollectives_with_workshop_invoices_related_to_one_FIN.sql
-- FraBe     11.11.21013 MKS-129221:1 / LOP2712: creation

-- reports all ServiceProvider - CollectiveInvoices which are connected to one FIN only


spool DataAnalysis_LOP2712-CostCollectives_with_workshop_invoices_related_to_one_FIN.log
...
variable L_SCRIPTNAME    varchar2 ( 120 char );
exec :L_SCRIPTNAME       := 'DataAnalysis_LOP2712-CostCollectives_with_workshop_invoices_related_to_one_FIN.sql';
...
select fzgre.ID_VERTRAG,        fzgre.ID_FZGVERTRAG
 ...
 where cov.COV_CAPTION not like 'MIG_OOS%'
   ...
   and not exists                                                  -- die verträge in der CostCollInv müssen alle dieselbe FIN haben
                                                                   -- ( -> check auf not exists eines vertrags in der CostCollInv mit anderer FIN )
     ( select /*+ index ( fzgv1 XPKTFZGVERTRAG ) index ( fzgre1 XIFSP_ID_FZGRE ) */  null
         from snt.TFZGVERTRAG            fzgv1
            , snt.TFZGRECHNUNG           fzgre1
        where fzgv1.ID_MANUFACTURE || fzgv1.FZGV_FGSTNR <> fzgv.ID_MANUFACTURE || fzgv.FZGV_FGSTNR
          and fzgv1.ID_VERTRAG       = fzgre1.ID_VERTRAG
          and fzgv1.ID_FZGVERTRAG    = fzgre1.ID_FZGVERTRAG
          and fzgre.GUID_SPCI        = fzgre1.GUID_SPCI )
order by 5, 3, 4, 1, 2;
...
prompt
prompt please contact the SIRIUS support team in Ulm if any ORA- or SP2- error is listed in the logfile DataAnalysis_LOP2712-CostCollectives_with_workshop_invoices_related_to_one_FIN.log
prompt
