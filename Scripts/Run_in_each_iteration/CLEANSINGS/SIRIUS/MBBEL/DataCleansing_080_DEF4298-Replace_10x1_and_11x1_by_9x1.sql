-- DataCleansing_080_DEF4298-Replace_10x1_and_11x1_by_9x1.sql
...
-- 2014-10-28; MARZUHL; V1.0; MKS-135396:1; Initial setup
...
--
   ...
   define GL_SCRIPTNAME         = DataCleansing_080_DEF4298-Replace_10x1_and_11x1_by_9x1
...
declare
	...
	CURSOR cur_SELECT is
		...
		from
			tcustomer
		where
			tcustomer.CUST_SAP_NUMBER_DEBITOR like '1111111111%'
		order by
			ID_CUSTOMER
		;
...
begin
	open cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_CUSTOMER;
		if cur_SELECT%NOTFOUND
		then
			if  cur_SELECT%ROWCOUNT <1
			then
				dbms_output.put_line ('INFO: No customers with more than 9x1 (10x1,11x1...) found.');
			end if;
			exit;
		end if;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('INFO: ID_CUSTOMER fixed to CUST_SAP_NUMBER_DEBITOR 9x1:');
		...
		update
			tcustomer
		set
			tcustomer.CUST_SAP_NUMBER_DEBITOR = '111111111'
		where
			tcustomer.ID_CUSTOMER = l_ID_CUSTOMER
			and tcustomer.CUST_SAP_NUMBER_DEBITOR like '1111111111%'
		;

		dbms_output.put_line ('INFO: ' || l_ID_CUSTOMER || ' MODIFIED.' );
	...
	exception
		when others then
			dbms_output.put_line ( 'ERR-: ' || l_ID_CUSTOMER || ' COULD NOT BE MODIFIED:' || sqlerrm );
