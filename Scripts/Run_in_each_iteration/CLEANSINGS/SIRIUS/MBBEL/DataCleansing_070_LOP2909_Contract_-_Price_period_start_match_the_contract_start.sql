-- DataCleansing_070_LOP2909_Contract_-_Price_period_start_match_the_contract_start.sql
...
-- 2014-07-01; MARZUHL; V1.0; MKS-133613:1; Initial Release
-- 2014-07-01; MARZUHL; V1.1; MKS-133613:2; FZGPR_PREIS_FIX wird zusätzlich mit 0 vorbelegt
-- ################################################################################################
--===================================================================================================================================
--===================================================================================================================================
...
--
   ...
   define GL_SCRIPTNAME		= DataCleansing_070_LOP2909_Contract_-_Price_period_start_match_the_contract_start
...
declare
	l_CONTRACT_NUMBER	varchar2 (50 char);
	...
	CURSOR cur_SELECT IS
		with table_mod_tfzgpreis as (
			select  distinct
				tfzgpreis.id_vertrag || '/' || tfzgpreis.ID_FZGVERTRAG	as CONTRACT_NUMBER
				, min (tfzgpreis.FZGPR_VON)
					over (
						partition by tfzgpreis.id_vertrag
						, tfzgpreis.ID_FZGVERTRAG
						)					as FIRST_PERIOD_BEGIN
			from
				tfzgvertrag
				, tfzgv_contracts
				, TDFCONTR_VARIANT
				, tfzgpreis
			where
				tfzgpreis.id_vertrag			= tfzgvertrag.id_vertrag
				and tfzgpreis.ID_FZGVERTRAG		= tfzgvertrag.ID_FZGVERTRAG
		...
		select
			table_mod_tfzgpreis.CONTRACT_NUMBER		as CONTRACT_NUMBER
			, tfzgv_contracts.ID_SEQ_FZGVC			as ID_SEQ_FZGVC
			, tfzgv_contracts.ID_VERTRAG			as ID_VERTRAG
			, tfzgv_contracts.ID_FZGVERTRAG			as ID_FZGVERTRAG
			, table_mod_tfzgpreis.FIRST_PERIOD_BEGIN	as FIRST_PERIOD_BEGIN
			, tfzgv_contracts.FZGVC_BEGINN			as FZGVC_BEGIN
		from
			table_mod_tfzgpreis
			, tfzgv_contracts
		where
			table_mod_tfzgpreis.CONTRACT_NUMBER		= tfzgv_contracts.id_vertrag || '/' || tfzgv_contracts.ID_FZGVERTRAG
			and tfzgv_contracts.FZGVC_BEGINN 		< table_mod_tfzgpreis.FIRST_PERIOD_BEGIN
		order by
			6
		;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_CONTRACT_NUMBER, l_ID_SEQ_FZGVC, l_ID_VERTRAG, l_ID_FZGVERTRAG, l_FIRST_PERIOD_BEGIN, l_FZGVC_BEGIN;
		if cur_SELECT%NOTFOUND
		then
			if  cur_SELECT%ROWCOUNT <1
			then
				dbms_output.put_line ('INFO: No contracts with FIRST_PERIOD_BEGIN > FZGVC_BEGIN found.');
			end if;
			exit;
		end if;
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('INFO: CONTRACT/NUMBER; FZGVC_BEGIN; FIRST_PERIOD_BEGIN; STATE');
		...
		values (
			...
			, 0
			, l_FZGVC_BEGIN
			, (l_FIRST_PERIOD_BEGIN - 1)
			, 0
			, 0
			)
		;
			
		dbms_output.put_line ('INFO: ' || l_CONTRACT_NUMBER || '; ' || l_FZGVC_BEGIN || '; ' || l_FIRST_PERIOD_BEGIN || '; MODIFIED' );
	...
	exception
		when others then
			dbms_output.put_line ( 'ERR-: ' || l_ID_VERTRAG || '/' || l_ID_FZGVERTRAG || '; ' || l_FZGVC_BEGIN || '; ' || l_FIRST_PERIOD_BEGIN || '; ' || sqlerrm );
