-- DataCleansing_LOPXXXX_-_FIX_Array_TVEGA_I55_CO_contains_no_values.sql
...
-- 2014-09-03; ZBerger; V1.2; MKS-133876:1; FIX - Array TVEGA_I55_CO contains no values
...
--
   ...
   define GL_SCRIPTNAME         = DataCleansing_LOPXXXX_-_FIX_Array_TVEGA_I55_CO_contains_no_values
...
-- main part for correcting code
begin

   dbms_output.put_line ('Inserting missing values in TVEGA_I55_CO');
   dbms_output.put_line ('========================================');

   for rec_missing in (select min.GUID_CONTRACT,
                              fzg.ID_VERTRAG,
                              fzg.ID_FZGVERTRAG
                         from (select FZG.GUID_CONTRACT
                                 from TFZGVERTRAG FZG
                                minus
                               select VIC.GUID_CONTRACT
                                 from TVEGA_I55_CO VIC) min,
                              tfzgvertrag fzg
                        where --ROWNUM < 100 and
                              fzg.guid_contract = min.guid_contract)
   loop
      begin
         --dbms_output.put_line (rec_missing.guid_contract);
         insert into snt.tvega_i55_co (GUID_I55_CO, GUID_VI55A, GUID_VI55AV, GUID_CONTRACT)
         select sys_guid(),
                via.GUID_VI55A,
                viav.GUID_VI55AV,
                rec_missing.GUID_CONTRACT
           from snt.TVEGA_I55_ATTRIBUTE   via,
                snt.TVEGA_I55_ATT_VALUE   viav
          where VI55A_SOURCE = 0
            and viav.GUID_VI55A = via.GUID_VI55A
            and viav.VI55AV_IS_DEFAULT_VALUE = 1;

         dbms_output.put_line ( 'Default-attributes entered for contract ' || rec_missing.id_vertrag || '-' || rec_missing.id_fzgvertrag );
         :L_DATASUCCESS_OCCURED := :L_DATASUCCESS_OCCURED + sql%rowcount;

      exception
         when dup_val_on_index then
          dbms_output.put_line ( 'Change was already done. - Change made not successful! Contract ' || rec_missing.id_vertrag || '-' || rec_missing.id_fzgvertrag );
          :L_DATAWARNINGS_OCCURED:= :L_DATAWARNINGS_OCCURED+1;
         
         when no_data_found then 
          dbms_output.put_line ( 'A Data error occured. - Change made not successful! Contract ' || rec_missing.id_vertrag || '-' || rec_missing.id_fzgvertrag );
          :L_DATAERRORS_OCCURED:= :L_DATAERRORS_OCCURED+1;

         when others then
          dbms_output.put_line ( 'A unhandled Data error occured. - Change made not successful! Contract ' || rec_missing.id_vertrag || '-' || rec_missing.id_fzgvertrag );
