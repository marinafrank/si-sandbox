                                     ...
                                     where c.ID_VERTRAG      = v.ID_VERTRAG
                                       ...
                                       and (((
                                             ( ('&&Scope' = 'A'
                                               ...
                                               and ( s.COS_STAT_CODE not in ( '00', '01', '02' ) and COALESCE (kmend.fzgkm_datum,c.FZGVC_ENDE ) < to_date ( '&&Out_Of_Scope_Date', 'DD.MM.YYYY' )
                                                     or 
                                                     s.COS_STAT_CODE =   '10' 
                                                   )
                                             )
    ...
    if '&&Scope' <> '0' then
      ...
      update snt.TFZGV_CONTRACTS c
       ...
       where ID_COV = l_id_cov_ins
          ...
          and decode(c.id_seq_fzgkmstand_end, null, FZGVC_ENDE, coalesce ((
                         select kmend.fzgkm_datum
                           from snt.tfzgkmstand     kmend
                          where c.id_seq_fzgkmstand_end = kmend.id_seq_fzgkmstand), FZGVC_ENDE
                    )
                    ) < to_date ( '&&Out_Of_Scope_Date', 'DD.MM.YYYY' )
          and ('&&Scope' = 'A' and
