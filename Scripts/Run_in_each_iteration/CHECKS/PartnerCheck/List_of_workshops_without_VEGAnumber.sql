-- List_of_workshops_without_VEGAnumber.sql
...
-- FraBe   11.03.2013 MKS-122926:1 creation
-- 2014-05-16; MARZUHL; V1.0; MKS-132551:1; Changed to new output format / framework
-- 2014-05-16; MARZUHL; V1.1; MKS-133622:1; Fix in "# affected" logic
...
--
   ...
   define GL_SCRIPTNAME		= List_of_workshops_without_VEGAnumber
...
declare
	...
	l_NAME_CAPTION1		varchar2 (50 char);
	l_NAME_TITEL1		varchar2 (50 char);
	l_ZIP_ZIP		varchar2 (50 char);
	l_ZIP_CITY		varchar2 (50 char);
	l_COU_CODE		varchar2 (50 char);
	...
	CURSOR cur_SELECT IS
		...
		from
			...
			, snt.VADRASSOZ		vadr
		where
			gar.ID_SEQ_ADRASSOZ			= vadr.ID_SEQ_ADRASSOZ
			and nvl(gar.GAR_IS_SERVICE_PROVIDER,0)	= 0		-- erste InScope bedingung für Workshops
			-- and gar.GAR_GARNOVEGA		<> '11924'	-- eigentlich wäre das die zweite bedingung für InScope für Workshops
										-- da dieses script aber alle InScope Workshops suchen, die keine VEGA nummer haben,
										-- kann diese bedingung nicht angegeben werden, sonst würden keine rows selektiert werden
										-- (-> das ergebnis von beiden GAR_GARNOVEGA abfragen ist ja nicht true! )
			and gar.GAR_GARNOVEGA			is null
		order by
			1
		;
...
begin
	...
	where
		gar.ID_SEQ_ADRASSOZ			= vadr.ID_SEQ_ADRASSOZ
		-- and nvl(gar.GAR_IS_SERVICE_PROVIDER,0)	= 0		-- erste InScope bedingung für Workshops -- Update: Deaktiviert aus 133622
		-- and gar.GAR_GARNOVEGA			<> '11924'	-- zweite InScope bedingung für Workshops -- Update: Deaktiviert aus 133622
	;

	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_GARAGE, l_NAME_CAPTION1, l_NAME_TITEL1, l_ZIP_ZIP, l_ZIP_CITY, l_COU_CODE;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; NAME_CAPTION1; NAME_TITEL1; ZIP_ZIP; ZIP_CITY; COU_CODE');
			dbms_output.put_line ('WARN: ------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE || '; ' ||l_NAME_CAPTION1 || '; ' || l_NAME_TITEL1 || '; ' || l_ZIP_ZIP || '; ' || l_ZIP_CITY || '; ' || l_COU_CODE);
