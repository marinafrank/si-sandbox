-- show_customers_with_second_phoneNumber_in_postalAdress.sql
...
-- FraBe  28.12.2012 MKS-121291:1 creation
-- FraBe  15.02.2013 MKS-122287:1 split into PrivateCustomer and commercialCustomer
--                                and do not log script anymore in the SIRIUS log
-- 2014-05-16; MARZUHL; V1.0; MKS-132558:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_customers_with_second_phoneNumber_in_postalAdress
...
declare
	l_ID_CUSTOMER		varchar2 (50 char);
	l_CUSTYP_COMPANY	varchar2 (50 char);
	l_NAME1_MATCHCODE	varchar2 (50 char);
	l_NAME1_TELEFON		varchar2 (50 char);
	l_NAME2_MATCHCODE	varchar2 (50 char);
	l_NAME2_TELEFON		varchar2 (50 char);

	CURSOR cur_SELECT IS
		select
			cust.ID_CUSTOMER
			, name1.NAME_MATCHCODE	as NAME1_MATCHCODE
			, name1.NAME_TELEFON	as NAME1_TELEFON
			, name2.NAME_MATCHCODE	as NAME2_MATCHCODE
			, name2.NAME_TELEFON	as NAME2_TELEFON
			, DECODE ( ctyp.CUSTYP_COMPANY 
				,'0', 'commercialCustomer'
				,'1', 'PrivateCustomer'
				,'2', 'commercialCustomer'
				, 'Data Error - Please Check!')	as CUSTYP_COMPANY
		from
			snt.TCUSTOMERTYP				ctyp
			, snt.TCUSTOMER					cust
			, snt.TADRASSOZ					adrass1
			, snt.TNAME					name1
			, snt.TADRASSOZ					adrass2
			, snt.TNAME					name2
		where
			ctyp.CUSTYP_COMPANY				in ( 0, 1, 2 )
			and ctyp.CUSTYP_CAPTION				not like 'MIG_OOS%'
			and ctyp.ID_CUSTYP				= cust.ID_CUSTYP
			and cust.ID_SEQ_ADRASSOZ			= adrass1.ID_SEQ_ADRASSOZ
			and name1.ID_SEQ_NAME				= adrass1.ID_SEQ_NAME
			and cust.ID_SEQ_ADRASSOZ2			= adrass2.ID_SEQ_ADRASSOZ (+)
			and name2.ID_SEQ_NAME 			(+)	= adrass2.ID_SEQ_NAME
			and name2.NAME_TELEFON				is not null
			and translate ( name2.NAME_TELEFON, '0()/.,- ', '0' ) <> nvl ( translate ( name1.NAME_TELEFON, '0()/.,- ', '0' ), ' ' )
			and instr ( nvl ( translate ( name1.NAME_TELEFON, '0()/.,- ', '0' ), ' ' ), translate ( name2.NAME_TELEFON, '0()/.,- ', '0' )) = 0
		order by
			6, 1;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_CUSTOMER, l_NAME1_MATCHCODE, l_NAME1_TELEFON, l_NAME2_MATCHCODE, l_NAME2_TELEFON, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; Fiscal Name Matchcode; Fiscal Phone; Postal Name Matchcode; Fiscal Phone');
			dbms_output.put_line ('WARN: ----------------------------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_NAME1_MATCHCODE || '; ' || l_NAME1_TELEFON || '; ' || l_NAME2_MATCHCODE || '; ' || L_NAME2_TELEFON);
