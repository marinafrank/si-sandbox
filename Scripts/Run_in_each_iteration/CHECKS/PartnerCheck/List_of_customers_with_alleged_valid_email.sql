-- List_of_customers_with_alleged_valid_email.sql
...
-- FraBe   11.12.2012 MKS-120603:1 creation
-- FraBe   14.12.2012 MKS-120603:2 add MIG_OOS condtion
...
-- 2014-05-16; MARZUHL	; V1.0; MKS-132549:2; Changed to new output format / framework
-- 2014-06-03; TK	; V1.1; MKS-133167:1; Changed to check for RegExp
...
--
   ...
   define GL_SCRIPTNAME		= List_of_customers_with_alleged_valid_email
...
declare
	...
	CURSOR cur_SELECT IS
		SELECT c.ID_CUSTOMER
		     , n.NAME_EMAIL AS EMAIL
		     , DECODE ( t.CUSTYP_COMPANY
		 ...
		 FROM tcustomer c,
 		      ...
 		      tname n,
 		      tcustomertyp t
 		WHERE  c.id_seq_adrassoz = ass.id_seq_adrassoz
 		  AND ass.id_seq_name = n.id_seq_name
 		  AND t.id_custyp = c.id_custyp
  		  AND t.custyp_caption NOT LIKE '%MIG_OOS%'
  		  AND NOT REGEXP_LIKE ( n.name_email, '[A-Za-z0-9_]+([-+.''][A-Za-z0-9_]+)*@[A-Za-z0-9_]+([-.][A-Za-z0-9_]+)*\.[A-Za-z0-9_]+([-.][A-Za-z0-9_]+)*' )
 		Order by 3,1;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_CUSTOMER, l_EMAIL, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; ID_CUSTOMER; EMAIL');
			dbms_output.put_line ('WARN: ----------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_ID_CUSTOMER || '; ' ||l_EMAIL);
