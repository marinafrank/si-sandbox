-- show_customers_with_empty_sap_debitor.sql
...
-- M.Zimmerberger   10.12.2012 MKS-120600:1 creation
-- FraBe            14.12.2012 MKS-120600:2 add MIG_OOS condtion / remove 2.8.0 check within SRS_LOG_MAINTENANCE_SCRIPTS - logging
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132556:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_customers_with_empty_sap_debitor
...
declare
	...
	CURSOR cur_SELECT IS
		...
		where
			...
			and cust.CUST_SAP_NUMBER_DEBITOR is null
		order by
			2, 1;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_CUSTOMER, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; ID_CUSTOMER;');
			dbms_output.put_line ('WARN: ----------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_ID_CUSTOMER );
