-- show_workshops_with_empty_zip_or_city.sql
...
-- FraBe   15.02.2013 MKS-122287:1 creation: almost copied from show_customers_with_empty_zip_or_city.sql
-- 2014-05-16; MARZUHL; V1.0; MKS-132562:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_workshops_with_empty_zip_or_city
...
declare
	...
	CURSOR cur_SELECT IS
		...
		from
			...
			, snt.TZIP		zip
		where
			adrass.ID_SEQ_ADRASSOZ		= gar.ID_SEQ_ADRASSOZ
			and adrass.ID_SEQ_ADRESS	= adr.ID_SEQ_ADRESS
			and zip.ID_SEQ_ZIP		= adr.ID_SEQ_ZIP
			and ( zip.ZIP_ZIP		is NULL
			 	or zip.ZIP_CITY         is NULL
				)
			and ( adr.ADR_STREET1		is NOT NULL
				or adr.ADR_STREET2      is NOT NULL
				)
		order by
			1
		;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_GARAGE, l_ZIP_ZIP, l_ZIP_CITY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; ZIP_ZIP; ZIP_CITY');
			dbms_output.put_line ('WARN: ----------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE || '; ' || l_ZIP_ZIP || '; ' || l_ZIP_CITY );
