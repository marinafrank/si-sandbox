-- List_of_workshops_with_alleged_valid_email.sql
...
-- FraBe   15.02.2013 MKS-122287:1 creation: almost copied from List_of_customers_with_alleged_valid_email.sql
-- 2014-05-16; MARZUHL	; V1.0; MKS-132552:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= List_of_workshops_with_alleged_valid_email
...
declare
	...
	CURSOR cur_SELECT IS
		select g.id_garage, n.name_email
		from tgarage g, tadrassoz ass, tname n
 		WHERE     g.id_seq_adrassoz = ass.id_seq_adrassoz
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_GARAGE, l_EMAIL;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; EMAIL');
			dbms_output.put_line ('WARN: ----------------');
		end if;
		dbms_output.put_line ('WARN: '|| l_ID_GARAGE || '; ' || l_EMAIL);
