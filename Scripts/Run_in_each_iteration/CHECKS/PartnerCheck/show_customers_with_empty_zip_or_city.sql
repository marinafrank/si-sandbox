-- show_customers_with_empty_zip_or_city.sql
...
-- M.Zimmerberger   10.12.2012 MKS-120601:1 creation
-- FraBe            13.12.2012 MKS-120601:1 add MIG_OOS condtion / remove 2.8.0 check within SRS_LOG_MAINTENANCE_SCRIPTS - logging
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132557:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_customers_with_empty_zip_or_city
...
declare
	...
	CURSOR cur_SELECT IS
		...
		where
			...
			and adrass.ID_SEQ_ADRESS    = adr.ID_SEQ_ADRESS
			and zip.ID_SEQ_ZIP          = adr.ID_SEQ_ZIP
			and ( zip.ZIP_ZIP          is     NULL  or zip.ZIP_CITY         is     NULL )
			and ( adr.ADR_STREET1      is NOT NULL  or adr.ADR_STREET2      is NOT NULL )
		order by
			4, 1;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_CUSTOMER, l_ZIP_ZIP, l_ZIP_CITY, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; ID_CUSTOMER; ZIP_ZIP; ZIP_CITY');
			dbms_output.put_line ('WARN: ----------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_ID_CUSTOMER || '; ' || l_ZIP_ZIP || '; ' || l_ZIP_CITY );
