-- DataAnalysis_LOP2748_Partner_show_all_customers_in_scope.sql
...
-- 2014-09-12; MARZUHL; V1.0; MKS-134866:1; Initial Setup
...
--
   ...
   define GL_SCRIPTNAME         = DataAnalysis_LOP2748_Partner_show_all_customers_in_scope
...
declare
	cursor c_partner_data is
		select
			...
			, TNAME.NAME_CAPTION1							as "namefield1"
			, nvl(TNAME.NAME_CAPTION2, '')						as "namefield2"
			, TNAME.NAME_CAPTION1
	...
	NAME_CAPTION2 varchar2(102 char);
	NAME_COMBINED varchar2(306 char);
...
begin
	open c_partner_data;
	loop
		fetch c_partner_data into ID_CUSTOMER, CUST_SAP_NUMBER_DEBITOR, NAME_CAPTION1, NAME_CAPTION2, NAME_COMBINED;
		if c_partner_data%NOTFOUND
		then
			if  c_partner_data%ROWCOUNT <1
			then
				dbms_output.put_line ( 'ERR: No partners found!');
		...
		if c_partner_data%ROWCOUNT = 1
		then
			dbms_output.put_line ('INFO: ID_CUSTOMER; CoFiCo; NAME_CAPTION1; NAME_CAPTION2; NAME_COMBINED (N_C1|N_C2|N_C3)');
			dbms_output.put_line ('INFO: =================================================================================');
		enD if;
		dbms_output.put_line ('INFO: ' || ID_CUSTOMER ||'; ' || CUST_SAP_NUMBER_DEBITOR || '; ' || NAME_CAPTION1 || '; ' || NAME_CAPTION2 || '; ' || NAME_COMBINED );
