-- List_of_customers_with_empty_salutation.sql
...
-- FraBe   11.12.2012 MKS-120599:1 creation
-- FraBe   13.12.2012 MKS-120599:2 add MIG_OOS condtion
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132550:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= List_of_customers_with_empty_salutation
...
declare
	...
	CURSOR cur_SELECT IS
		...
		where
			...
			and vadr.ID_SEQ_ADRASSOZ	= cust.ID_SEQ_ADRASSOZ
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_CUSTOMER, l_ID_TITLE, l_TITLE, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; ID_CUSTOMER; ID_TITLE; TITLE');
			dbms_output.put_line ('WARN: --------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_ID_CUSTOMER || '; ' ||l_ID_TITLE || '; ' ||l_TITLE);
