-- show_customerParent_with_all_parent_relations.sql
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132554:1; Changed to new output format / framework. Splitted into two scripts. One with all parent relations (this one), one with only in scope and same type as customer.
-- 2014-06-30; MARZUHL; V1.1; MKS-133622:1; Modification to weighting (Data-Warnings vs. Data-Erros and Affected)
...
--
   ...
   define GL_SCRIPTNAME		= show_customerParent_with_all_parent_relations
...
declare
	l_ID_CUSTOMER_FILIALE	varchar2 (50 char);
	l_NAME1_CAPTION1	varchar2 (50 char);
	l_NAME1_CAPTION2	varchar2 (50 char);
	l_ADR1_STREET1		varchar2 (50 char);
	l_ZIP1_ZIP		varchar2 (50 char);
	l_ZIP1_CITY		varchar2 (50 char);
	l_ID_CUSTOMER_ZENTRALE	varchar2 (50 char);
	l_NAME2_CAPTION1	varchar2 (50 char);
	l_NAME2_CAPTION2	varchar2 (50 char);
	l_ADR2_STREET1		varchar2 (50 char);
	l_ZIP2_ZIP		varchar2 (50 char);
	l_ZIP2_CITY		varchar2 (50 char);


	CURSOR cur_SELECT IS
		select
			cust.ID_CUSTOMER           as ID_CUSTOMER_FILIALE  -- Customer in scope with all  (in/out of scope) parent relation
			, vadr.NAME_CAPTION1
			, vadr.NAME_CAPTION2
			, vadr.ADR_STREET1
			, vadr.ZIP_ZIP
			, vadr.ZIP_CITY
			, cust.ID_CUSTOMER_PARENT    as ID_CUSTOMER_ZENTRALE
			, vadrParent.NAME_CAPTION1
			, vadrParent.NAME_CAPTION2
			, vadrParent.ADR_STREET1
			, vadrParent.ZIP_ZIP
			, vadrParent.ZIP_CITY
		from
		...
		where
			vadrParent.ID_SEQ_ADRASSOZ  = custParent.ID_SEQ_ADRASSOZ
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_CUSTOMER_FILIALE, l_NAME1_CAPTION1, l_NAME1_CAPTION2, l_ADR1_STREET1, l_ZIP1_ZIP, l_ZIP1_CITY, l_ID_CUSTOMER_ZENTRALE, l_NAME2_CAPTION1, l_NAME2_CAPTION2, l_ADR2_STREET1, l_ZIP2_ZIP, l_ZIP2_CITY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_CUSTOMER_FILIALE; NAME1_CAPTION1; NAME_CAPTION2; ADR_STREET1; ZIP_ZIP; ZIP_CITY; ID_CUSTOMER_ZENTRALE; NAME_CAPTION1; NAME_CAPTION2; ADR_STREET1; ZIP_ZIP; ZIP_CITY');
			dbms_output.put_line ('WARN: ----------------------------------------------------------------------------------------------------------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_CUSTOMER_FILIALE || '; ' || l_NAME1_CAPTION1 || '; ' || l_NAME1_CAPTION2 || '; ' || l_ADR1_STREET1 || '; ' || l_ZIP1_ZIP || '; ' || l_ZIP1_CITY || '; ' || l_ID_CUSTOMER_ZENTRALE || '; ' || l_NAME2_CAPTION1 || '; ' || l_NAME2_CAPTION2 || '; ' || l_ADR2_STREET1 || '; ' || l_ZIP2_ZIP || '; ' || l_ZIP2_CITY);
