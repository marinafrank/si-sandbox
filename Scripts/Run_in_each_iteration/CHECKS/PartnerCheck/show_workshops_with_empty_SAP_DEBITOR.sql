-- show_workshops_with_empty_sap_debitor.sql
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132560:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_workshops_with_empty_sap_debitor
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_GARAGE;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE has empty SAP Debitor');
			dbms_output.put_line ('WARN: -------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE);
