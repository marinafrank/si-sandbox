-- show_workshops_with_empty_sap_debitor_or_creditor.sql
...
-- FraBe   15.02.2013 MKS-122287:1 creation: almost copied from show_customers_with_empty_sap_debitor.sql
-- FraBe   20.03.2013 MKS-122286:1 add creditor check
-- 2014-05-16; MARZUHL; V1.0; MKS-132561:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_workshops_with_empty_sap_debitor_or_creditor
...
declare
	...
	l_GAR_FI_DEBITOR	varchar2 (50 char);
	l_GAR_FI_CREDITOR	varchar2 (50 char);
	...
	CURSOR cur_SELECT IS
		...
		from
			snt.TGARAGE        gar
		where
			gar.GAR_FI_DEBITOR	is null
			or gar.GAR_FI_CREDITOR	is null
		order by
			1
		;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_GARAGE, l_GAR_FI_DEBITOR, l_GAR_FI_CREDITOR;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; GAR_FI_DEBITOR; GAR_FI_CREDITOR');
			dbms_output.put_line ('WARN: ------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE || '; ' || l_GAR_FI_DEBITOR || '; ' || l_GAR_FI_CREDITOR );
