-- show_workshops_with_bad_phone_or_fax.sql
...
-- FraBe   15.02.2013 MKS-122287:1 creation: almost copied from show_customers_with_bad_phone_or_fax.sql
-- 2014-05-16; MARZUHL; V1.0; MKS-132559:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_workshops_with_bad_phone_or_fax
...
declare
	...
	CURSOR cur_SELECT IS
		...
		where
			...
			and ( length ( name.NAME_TELEFON ) > 15 
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_GARAGE, l_NAME_TELEFON, l_NAME_FAX;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; NAME_TELEFON; NAME_FAX');
			dbms_output.put_line ('WARN: ----------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE || '; ' || l_NAME_TELEFON || '; ' || l_NAME_FAX );
