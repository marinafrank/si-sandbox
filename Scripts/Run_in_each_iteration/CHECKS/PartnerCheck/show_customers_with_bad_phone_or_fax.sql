-- show_customers_with_bad_phone_or_fax.sql
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132555:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_customers_with_bad_phone_or_fax
...
declare
	...
	l_NAME_TELEFON		varchar2 (50 char);
	l_NAME_FAX		varchar2 (50 char);
	...
	CURSOR cur_SELECT IS
		...
		where
			...
			and (
				length ( name.NAME_TELEFON )		> 15 
				or length ( name.NAME_FAX )		> 15 
				or  instr ( name.NAME_TELEFON, 'gsm' )	> 0 
				or  instr ( name.NAME_FAX,     'gsm' )	> 0
				)
		order by
			4, 1;
...
begin
	...
	OPEN cur_SELECT;
	LOOP
	fetch cur_SELECT into l_ID_CUSTOMER, l_NAME_TELEFON, l_NAME_FAX, l_CUSTYP_COMPANY;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CUSTYP_COMPANY; ID_CUSTOMER; NAME_TELEFON; NAME_FAX');
			dbms_output.put_line ('WARN: ---------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_CUSTYP_COMPANY || '; ' || l_ID_CUSTOMER || '; ' || l_NAME_TELEFON || '; ' || l_NAME_FAX);
