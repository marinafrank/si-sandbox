...
-- 2015-02-18; MARZUHL/MARINF; V1.0; MKS-136453:1; Initial Release
...
--
   ...
   define GL_SCRIPTNAME        = DataAnalysis_DEF7813_Contracts_with_product_MIGRATION
...
declare
	l_ID_VERTRAG		TFZGV_CONTRACTS.ID_VERTRAG@simex_db_link%type;
	l_ID_FZGVERTRAG		TFZGV_CONTRACTS.ID_FZGVERTRAG@simex_db_link%type;
	l_PRODUCT		VARCHAR2(50CHAR);
	cursor cur_SELECT is
		...
		from (
			...
			, PCK_CONTRACT.get_Product_House (
				I_PH_TYPE		=> 'PRODUCT'
				, I_PH_PARAM_IN1	=> ICP_CAPTION
				, I_PH_PARAM_IN2	=> end_ID_COV
				, I_PH_PARAM_IN3	=> PCK_CONTRACT.get_Used (
					I_FZGV_ERSTZULASSUNG => FZGV_ERSTZULASSUNG
					, I_FZGVC_BEGINN       => start_FZGVC_BEGINN )
					, I_RETURN_WHAT  => 'PH_PARAM_OUT1' )		PRODUCT
			from (
				SELECT
					fv.ID_VERTRAG
					, fv.ID_FZGVERTRAG
					, fv.GUID_CONTRACT
					, fv.FZGV_ERSTZULASSUNG
					, fv.ID_MANUFACTURE
					, fv.FZGV_FGSTNR
					, (	SELECT
							icp.ICP_CAPTION
						...
						WHERE
							COpack.GUID_CONTRACT = fv.GUID_CONTRACT
							AND ROWNUM = 1)				icp_caption
					, row_number() OVER (PARTITION BY fc.ID_VERTRAG,fc.ID_FZGVERTRAG ORDER BY fc.ID_FZGVERTRAG)			rn
					, min ( fc.FZGVC_BEGINN)
						OVER (PARTITION BY fc.ID_VERTRAG, fc.ID_FZGVERTRAG)							start_FZGVC_BEGINN
					, last_value (fc.ID_COV)
						OVER (PARTITION BY fc.ID_VERTRAG, fc.ID_FZGVERTRAG 
						ORDER BY fc.FZGVC_BEGINN, fc.ID_SEQ_FZGVC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)	end_ID_COV
					, last_value (fc.FZGVC_ENDE)
						OVER (PARTITION BY fc.ID_VERTRAG, fc.ID_FZGVERTRAG 
						ORDER BY fc.FZGVC_BEGINN, fc.ID_SEQ_FZGVC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)	end_FZGVC_ENDE
				from
					snt.TFZGVERTRAG@SIMEX_DB_LINK fv 
					JOIN snt.TFZGV_CONTRACTS@SIMEX_DB_LINK			fc
						ON fv.ID_VERTRAG = fc.ID_VERTRAG
						AND fv.id_fzgvertrag = fc.id_fzgvertrag
					JOIN snt.TDFCONTR_VARIANT@SIMEX_DB_LINK			cov
						ON cov.ID_COV    = fc.ID_COV AND cov.COV_CAPTION not like 'MIG_OOS%'
				) f
			where
				f.rn=1
			)
...
begin
	...
	from
		TFZGV_CONTRACTS@simex_db_link join tdfcontr_variant@simex_db_link using (id_cov)
	where
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_ID_VERTRAG, l_ID_FZGVERTRAG, l_PRODUCT;
		if cur_SELECT%NOTFOUND
		then
			if  cur_SELECT%ROWCOUNT <1
			then
				dbms_output.put_line ('ERR : No data found.');
			end if;
			exit;
		end if;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_VERTRAG/ID_FZGVERTRAG; PRODUCT');
			dbms_output.put_line ('WARN: ---------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_VERTRAG || '/' ||l_ID_FZGVERTRAG || '; ' || l_PRODUCT);
