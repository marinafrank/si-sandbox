-- DataAnalysis_LOP2721_AtLeastOneWorkhopInvoiceWithaHigherMileageThanThisDurationRealEndMileage.sql
...
-- M.Zimmerberger	29.10.2012 MKS-119329:1 creation
-- FraBe          30.10.2012 MKS-119329:2 use to_char within km.fzgkm
--                                        no log im SIRIUS log
-- M.Zimmerberger	19.03.2013 MKS-122555:1 consider migration-scope MIG_OOS
-- 2014-05-16; MARZUHL; V1.0; MKS-132548:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= DataAnalysis_LOP2721_AtLeastOneWorkhopInvoiceWithaHigherMileageThanThisDurationRealEndMileage
...
declare
	...
	l_FZGKM_DATUM		varchar2 (50 char);
	l_FZGKM_KM		varchar2 (50 char);
	l_ID_SEQ_FZGRECHNUNG	varchar2 (50 char);
	l_FZGRE_BELEGDATUM	varchar2 (50 char);
	l_FZGRE_LAUFSTRECKE	varchar2 (50 char);
	l_ID_GARAGE		varchar2 (50 char);
	l_NAME_MATCHCODE	varchar2 (50 char);
	l_BELART_CAPTION	varchar2 (50 char);
	l_FZGRE_RESUMME		varchar2 (50 char);
	l_FZGRE_REFERENZBUCHUNG	varchar2 (50 char);
	l_FZGRE_MEMO		varchar2 (500 char);


	CURSOR cur_SELECT IS
		select
			fzgvc.ID_VERTRAG								-- "CO"
			, fzgvc.ID_FZGVERTRAG								-- "POS"
			, to_char ( km.FZGKM_DATUM, 'DD.MM.YYYY' ) 		as FZGKM_DATUM		-- "CO FinalEndDate"
			, to_char ( km.FZGKM_KM )				as FZGKM_KM		-- "CO FinalEndKM"
			, to_char ( fzgre.ID_SEQ_FZGRECHNUNG )			as ID_SEQ_FZGRECHNUNG	-- "SIRIUS Inv/CN ID"
			, to_char ( fzgre.FZGRE_BELEGDATUM, 'DD.MM.YYYY' )	as FZGRE_BELEGDATUM	-- "LastWorkshopInvDate"
			, to_char ( fzgre.FZGRE_LAUFSTRECKE )			as FZGRE_LAUFSTRECKE	-- "LastWorkshopInvKM"
			, to_char ( gar.ID_GARAGE )				as ID_GARAGE		-- "ID Garage"
			, vgar.NAME_MATCHCODE								-- "Name Matchcode"
			, belart.BELART_CAPTION								-- "Inv/CN Type"
			, to_char ( fzgre.FZGRE_RESUMME, 'FM9999990D00' )	as FZGRE_RESUMME	-- "Inv/CN Amount"
			, to_char ( fzgre.FZGRE_REFERENZBUCHUNG )		as FZGRE_REFERENZBUCHUNG -- "RefersTo SIRIUS Inv/CN ID"
			, replace ( replace ( fzgre.FZGRE_MEMO, chr(10), ' ' ), chr(13), ' ' )		-- "Memo"	
		from
		...
		where
			cvar.COV_CAPTION		not like 'MIG_OOS%'
			and fzgvc.ID_COV		= cvar.ID_COV
			and fzgvc.ID_SEQ_FZGVC		= get_max_CO ( fzgvc.ID_VERTRAG, fzgvc.ID_FZGVERTRAG )
			and fzgvc.ID_SEQ_FZGVC		= fzgre.ID_SEQ_FZGVC
			and fzgvc.ID_SEQ_FZGKMSTAND_END	= km.ID_SEQ_FZGKMSTAND
			and fzgre.FZGRE_LAUFSTRECKE	> km.FZGKM_KM               -- mit WorkshopInv - KM nach dem CO FinalEnd
			and fzgre.ID_BELEGART		= belart.ID_BELEGART
			and fzgre.GUID_PARTNER		= part.GUID_PARTNER
			and gar.ID_GARAGE		= part.ID_GARAGE
			and gar.ID_SEQ_ADRASSOZ		= vgar.ID_SEQ_ADRASSOZ
			and belart.id_belegart		not in (4,6,88,89)
		order by
			1, 2
		;
...
begin
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_id_vertrag, l_id_fzgvertrag, l_FZGKM_DATUM, l_FZGKM_KM, l_ID_SEQ_FZGRECHNUNG, l_FZGRE_BELEGDATUM, l_FZGRE_LAUFSTRECKE, l_ID_GARAGE, l_NAME_MATCHCODE, l_BELART_CAPTION, l_FZGRE_RESUMME, l_FZGRE_REFERENZBUCHUNG, l_FZGRE_MEMO;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CONTRACT/FZGC; CO FinalEndDate; CO FinalEndKM; SIRIUS Inv/CN ID; LastWorkshopInvDate; LastWorkshopInvKM; ID Garage; Name Matchcode; Inv/CN Type; Inv/CN Amount; RefersTo SIRIUS Inv/CN ID; Memo');
			dbms_output.put_line ('WARN: -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_id_vertrag || '/' || l_id_fzgvertrag || '; ' ||l_FZGKM_DATUM || '; ' ||l_FZGKM_KM || '; ' ||l_ID_SEQ_FZGRECHNUNG || '; ' ||l_FZGRE_BELEGDATUM || '; ' ||l_FZGRE_LAUFSTRECKE || '; ' ||l_ID_GARAGE || '; ' ||l_NAME_MATCHCODE || '; ' ||l_BELART_CAPTION || '; ' ||l_FZGRE_RESUMME || '; ' ||l_FZGRE_REFERENZBUCHUNG || '; ' ||l_FZGRE_MEMO);
