-- 20121231_checkContractDurationsInFuture_V1.0.sql
...
-- FraBe  31.12.2012 MKS-121064:1 creation
-- 2014-05-16; MARZUHL; V1.0; MKS-132544:2; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= 20121231_checkContractDurationsInFuture_V1.0
...
declare
	...
	l_fin		varchar2 (50 char);
	l_FZGVC_BEGINN	varchar2 (50 char);
	l_FZGVC_ENDE	varchar2 (50 char);

	CURSOR cur_SELECT IS
		select
			fzgv.ID_VERTRAG
			, fzgv.ID_FZGVERTRAG
			, fzgv.ID_MANUFACTURE || fzgv.FZGV_FGSTNR      as FIN
			, to_char ( fzgvc.FZGVC_BEGINN, 'DD.MM.YYYY' ) as FZGVC_BEGINN
			, to_char ( fzgvc.FZGVC_ENDE,   'DD.MM.YYYY' ) as FZGVC_ENDE
		from
			snt.TDFCONTR_VARIANT	cvar
			, snt.TFZGV_CONTRACTS	fzgvc
			, snt.TFZGVERTRAG	fzgv
		where
			fzgvc.ID_VERTRAG	= fzgv.ID_VERTRAG
			and fzgvc.ID_FZGVERTRAG	= fzgv.ID_FZGVERTRAG
			and fzgvc.FZGVC_BEGINN	> sysdate
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_id_vertrag, l_id_fzgvertrag, l_fin, l_FZGVC_BEGINN, l_FZGVC_ENDE;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CONTRACT/FZGC; FIN; CONTRACT BEGIN; CONTRACT END');
			dbms_output.put_line ('WARN: ------------------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_id_vertrag || '/' || l_id_fzgvertrag || '; ' || l_fin || '; ' || l_FZGVC_BEGINN || '; ' || l_FZGVC_ENDE );
