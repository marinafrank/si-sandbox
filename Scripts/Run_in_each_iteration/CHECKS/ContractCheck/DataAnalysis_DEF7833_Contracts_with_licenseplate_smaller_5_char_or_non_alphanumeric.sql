...
-- 2015-02-25; MARZUHL; V1.0; MKS-136455:1; Initial Release
...
--
   ...
   define GL_SCRIPTNAME		= DataAnalysis_DEF7833_Contracts_with_licenseplate_smaller_5_char_or_non_alphanumeric
...
declare
	...
	l_FZGV_KFZKENNZEICHEN	varchar2 (50 char);
	...
	CURSOR cur_SELECT IS
		...
		FROM
			tfzgvertrag
			join tfzgv_contracts using (id_vertrag, id_fzgvertrag)
			join tdfcontr_variant using (id_cov)
		WHERE
			cov_caption not like 'MIG_OOS%'
			and (
				REGEXP_LIKE(UPPER(FZGV_KFZKENNZEICHEN), '[^A-Z,^0-9]')
				or length(FZGV_KFZKENNZEICHEN) < 5
			)
		order by
			id_vertrag
			, id_fzgvertrag
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_id_vertrag, l_id_fzgvertrag, l_FZGV_KFZKENNZEICHEN;
		if cur_SELECT%NOTFOUND
		then
			if  cur_SELECT%ROWCOUNT <1
			then
				dbms_output.put_line ('INFO: All contract numberplates are alphanumeric and longer than 5 characters.');
			end if;
			exit;
		end if;
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CONTRACT/FZGC; NUMBERPLATE');
			dbms_output.put_line ('WARN: --------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_id_vertrag || '/' || l_id_fzgvertrag || '; ' ||l_FZGV_KFZKENNZEICHEN);
