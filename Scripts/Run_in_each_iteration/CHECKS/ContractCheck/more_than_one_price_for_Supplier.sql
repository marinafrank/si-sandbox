-- more_than_one_price_for_Supplier.sql
...
-- FraBe     07.01.2013 MKS-121109:1 creation
-- MaZi      18.03.2013 MKS-122554:1 consider migration-scope
-- 2014-05-16; MARZUHL; V1.0; MKS-132546:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= more_than_one_price_for_Supplier
...
declare
	...
	l_VALID_FROM		varchar2 (50 char);
	l_VALID_TO		varchar2 (50 char);
	l_PRICE_VALUE		varchar2 (50 char);
	l_RP_MILEAGE		varchar2 (50 char);
	

	CURSOR cur_SELECT IS
		select
			c.ID_VERTRAG || c.ID_FZGVERTRAG as ID_VERTRAG_ID_FZGVERTRAG
			, g.ID_GARAGE
			, a.NAME_MATCHCODE
			, c.ID_VERTRAG
			, c.ID_FZGVERTRAG
			, to_char ( pri.SPCP_VALID_FROM, 'DD.MM.YYYY' ) as VALID_FROM
			, to_char ( pri.SPCP_VALID_TO,   'DD.MM.YYYY' ) as VALID_TO
			, to_char ( pri.SPCP_VALUE, '999,990.00' )      as PRICE_VALUE
			, to_char ( pri.SPCP_RP_MILEAGE, '999,990.00' ) as RP_MILEAGE
		from
			snt.VADRASSOZ          a
			, snt.TGARAGE            g
			, snt.TPARTNER           p
			, snt.TSP_CONTRACT_PRICE pri
			, snt.TSP_CONTRACT       c
			, snt.TFZGV_CONTRACTS    fzgv
			, snt.TDFCONTR_VARIANT   cov
			, snt.TFZGVERTRAG        fzg     
		where
			c.GUID_SPC		= pri.GUID_SPC
			and c.GUID_PARTNER	= p.GUID_PARTNER
			and g.ID_GARAGE		= p.ID_GARAGE
			and g.ID_SEQ_ADRASSOZ	= a.ID_SEQ_ADRASSOZ
			and fzg.ID_VERTRAG	= c.ID_VERTRAG
			and fzg.ID_FZGVERTRAG	= c.ID_FZGVERTRAG   
			and fzgv.ID_VERTRAG	= fzg.ID_VERTRAG
			and fzgv.ID_FZGVERTRAG	= fzg.ID_FZGVERTRAG
			...
			and ( c.GUID_PARTNER
				...
				,  c.ID_FZGVERTRAG ) in
					( select
						...
						, c1.ID_FZGVERTRAG
					from
						snt.TSP_CONTRACT_PRICE	pri1
						, snt.TSP_CONTRACT	c1
					where
						c1.GUID_SPC	= pri1.GUID_SPC
					group by
						c1.GUID_PARTNER
						, c1.ID_VERTRAG
						, c1.ID_FZGVERTRAG
...
begin
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_waste, l_ID_GARAGE, l_NAME_MATCHCODE, l_ID_VERTRAG, l_ID_FZGVERTRAG, l_VALID_FROM, l_VALID_TO, l_PRICE_VALUE, l_RP_MILEAGE;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: ID_GARAGE; NAME_MATCHCODE; ID_VERTRAG/FZGV; VALID_FROM; VALID_TO; PRICE_VALUE; RP_MILEAGE');
			dbms_output.put_line ('WARN: -----------------------------------------------------------------------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_ID_GARAGE || '; ' || l_NAME_MATCHCODE || '; ' || l_id_vertrag || '/' || l_ID_FZGVERTRAG || '; ' || l_VALID_FROM || '; ' || l_VALID_TO || '; ' || l_PRICE_VALUE || '; ' || l_RP_MILEAGE );
