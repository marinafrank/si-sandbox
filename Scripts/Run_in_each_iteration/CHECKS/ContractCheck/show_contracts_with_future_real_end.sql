-- show_contracts_with_future_real_end.sql
...
-- 2014-05-16; MARZUHL; V1.0; MKS-132547:1; Changed to new output format / framework
...
--
   ...
   define GL_SCRIPTNAME		= show_contracts_with_future_real_end
...
declare
	...
	l_real_end_date	varchar2 (50 char);

	CURSOR cur_SELECT IS
		SELECT
			km.id_vertrag
			,km.id_fzgvertrag
			,TO_CHAR (km.fzgkm_datum, 'DD.MM.YYYY')	as real_end_date
		FROM
			tfzgkmstand		km
			, tfzgv_contracts	fzgv
			, tdfcontr_variant	cov
 		WHERE
			km.fzgkm_datum > SYSDATE
			AND fzgv.id_seq_fzgvc = km.id_seq_fzgvc
			...
			AND EXISTS
				...
				FROM
					snt.tfzgv_contracts
				WHERE
					id_seq_fzgkmstand_end = km.id_seq_fzgkmstand)
		;
...
begin
	...
	FROM
		...
		, tdfcontr_variant	cov
 	WHERE
		fzgv.id_seq_fzgvc = km.id_seq_fzgvc
		AND fzgv.id_cov = cov.id_cov
		AND cov.cov_caption NOT LIKE '%MIG_OOS%'
		AND EXISTS
			(SELECT
	...
	OPEN cur_SELECT;
	LOOP
		fetch cur_SELECT into l_id_vertrag, l_id_fzgvertrag, l_real_end_date;
		...
		if cur_SELECT%ROWCOUNT = 1
		then
			dbms_output.put_line ('WARN: CONTRACT/FZGC; Real End Date');
			dbms_output.put_line ('WARN: ----------------------------');
		end if;
		dbms_output.put_line ('WARN: ' || l_id_vertrag || '/' || l_id_fzgvertrag || '; ' ||l_real_end_date);
